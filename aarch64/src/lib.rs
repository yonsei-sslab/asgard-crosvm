...
const AARCH64_PHYS_MEM_START_SECOND: u64 = 0xa0000000;
const AARCH64_AXI_BASE: u64 = 0x40000000;

// HACK: The platform MMIO region is *not* placed after the end of guest RAM.
const AARCH64_PLATFORM_MMIO_START: u64 = 0xf0000000;
...
const AARCH64_PROTECTED_VM_FW_START_SECOND: u64 =
    AARCH64_PHYS_MEM_START_SECOND - AARCH64_PROTECTED_VM_FW_MAX_SIZE;

pub const AARCH64_INFERENCE_IO_MEM_MAX_SIZE: u64 = 0x4000000;
pub const AARCH64_INFERENCE_IO_MEM_START: u64 = AARCH64_PHYS_MEM_START -
    AARCH64_PROTECTED_VM_FW_MAX_SIZE - AARCH64_PVTIME_IPA_MAX_SIZE - AARCH64_INFERENCE_IO_MEM_MAX_SIZE;
pub const AARCH64_INFERENCE_IO_MEM_START_SECOND: u64 = AARCH64_PHYS_MEM_START -
    AARCH64_PROTECTED_VM_FW_MAX_SIZE - AARCH64_PVTIME_IPA_MAX_SIZE - AARCH64_INFERENCE_IO_MEM_MAX_SIZE;
...
const AARCH64_PVTIME_IPA_START_SECOND: u64 = AARCH64_PROTECTED_VM_FW_START_SECOND - AARCH64_PVTIME_IPA_MAX_SIZE;
...
fn get_second_kernel_addr() -> GuestAddress {
    GuestAddress(AARCH64_PHYS_MEM_START_SECOND + AARCH64_KERNEL_OFFSET)
}
...
fn get_second_bios_addr() -> GuestAddress {
    GuestAddress(AARCH64_PHYS_MEM_START_SECOND + AARCH64_BIOS_OFFSET)
}
...
pub fn arch_second_memory_regions(size: u64) -> Vec<(GuestAddress, u64)> {
    vec![(GuestAddress(AARCH64_PHYS_MEM_START_SECOND), size)]
}
...
impl arch::LinuxArch for AArch64 {
    ...
    fn guest_memory_layout(
        ...
        if components.second_vm {
            Ok(arch_second_memory_regions(components.memory_size))
        } else {
        ...
        Vcpu: VcpuAArch64,
    {
        ...
        let image_size = match components.vm_image {
            VmImage::Bios(ref mut bios) => {
                let bios_addr = if components.second_vm {
                    get_second_bios_addr()
                } else {
                    get_bios_addr()
                };
                arch::load_image(&mem, bios, bios_addr, AARCH64_BIOS_MAX_LEN)
                    .map_err(Error::BiosLoadFailure)?
            }
            VmImage::Kernel(ref mut kernel_image) => {
                let kernel_addr = if components.second_vm {
                    get_second_kernel_addr()
                } else {
                    get_kernel_addr()
                };
                let kernel_size =
                ...
                let kernel_end = kernel_addr.offset() + kernel_size as u64;
                initrd = match components.initrd_image {
                    Some(initrd_file) => {
                        ...
                        let initrd_max_size = if components.second_vm {
                            components.memory_size - (initrd_addr - AARCH64_PHYS_MEM_START_SECOND)
                        } else {
        ...
        if has_pvtime {
            ...
            if components.second_vm {
                vm.add_memory_region(
                    GuestAddress(AARCH64_PVTIME_IPA_START_SECOND),
                    Box::new(pvtime_mem),
                    false,
                    false,
                    false,
        ...
        if components.protected_vm == ProtectionType::Protected {
            if components.second_vm {
                vm.load_protected_vm_firmware(
                    GuestAddress(AARCH64_PROTECTED_VM_FW_START_SECOND),
        ...
        for (vcpu_id, vcpu) in vcpus.iter().enumerate() {
            ...
            if has_pvtime {
                if components.second_vm {
                    vcpu.init_pvtime(AARCH64_PVTIME_IPA_START_SECOND + (vcpu_id as u64 * AARCH64_PVTIME_SIZE))
        ...
        let platform_devices = platform_devices
            ...
            .collect();
        let (platform_info, mut platform_pid_debug_label_map) = arch::generate_platform_bus(
            platform_devices,
            irq_chip.as_irq_chip_mut(),
            &mmio_bus,
            system_allocator,
        ...
        let (plat_mmio_base, plat_mmio_size) =
            Self::get_platform_mmio_base_size(components.memory_size);
...
impl AArch64 {
    fn get_platform_mmio_base_size(mem_size: u64) -> (u64, u64) {
        // HACK: The platform MMIO region is *not* placed after the end of guest RAM.
        let base = AARCH64_PLATFORM_MMIO_START;
    ...
    fn get_resource_allocator_config(
        ...
        // HACK: Check whether the guest RAM exceeds the platform MMIO start address.
        if (AARCH64_PHYS_MEM_START + mem_size) > AARCH64_PLATFORM_MMIO_START {
            panic!("guest memory size is too big!");
        }
        if (AARCH64_PHYS_MEM_START_SECOND + mem_size) > AARCH64_PLATFORM_MMIO_START {
            panic!("guest memory size is too big!");
        }
        let guest_phys_end = 1u64 << guest_phys_addr_bits;
        // HACK: The platform MMIO region is *not* placed after the end of guest RAM.
        let plat_mmio_base = AARCH64_PLATFORM_MMIO_START;
    ...
    fn configure_vcpu_early(
        ...
        if vcpu_id == 0 {
            ...
            let fdt_addr = if second_vm {
                (AARCH64_PHYS_MEM_START_SECOND + fdt_offset(mem_size, has_bios))
            } else {
                (AARCH64_PHYS_MEM_START + fdt_offset(mem_size, has_bios))
            } as u64;
