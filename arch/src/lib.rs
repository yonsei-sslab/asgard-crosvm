...
pub fn generate_platform_bus(
    ...
    mmio_bus: &Bus,
    resources: &mut SystemAllocator,
) -> Result<(Vec<(String, Vec<(u64, u64)>, Vec<(u32, u32)>)>, BTreeMap<u32, String>), DeviceRegistrationError> {
    ...
    for (mut device, jail) in devices.into_iter() {
        ...
        for irq in irqs.into_iter() {
            ...
            if device.irq_is_automask(&irq) {
                ...
                irq_info.push((irq_num, irq.flags));
            } else {
                let irq_evt =
        ...
        let sysfspath = device.get_platform_sysfspath().clone();
        let arced_dev: Arc<Mutex<dyn BusDevice>> = if let Some(jail) = jail {
            let proxy = ProxyDevice::new(device, &jail, keep_rds)
                .map_err(DeviceRegistrationError::ProxyDeviceCreation)?;
        ...
        platform_info.push((sysfspath, ranges, irq_info));
    }
    Ok((platform_info, pid_labels))
}
