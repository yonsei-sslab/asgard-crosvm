...
use {
    ...
    aarch64::AARCH64_INFERENCE_IO_MEM_START,
    aarch64::AARCH64_INFERENCE_IO_MEM_START_SECOND,
    aarch64::AARCH64_INFERENCE_IO_MEM_MAX_SIZE,
};
...
pub fn run_config(cfg: Config) -> Result<ExitState> {
    ...
    let mut vm = KvmVm::new(&kvm, guest_mem, components.protected_vm).context("failed to create vm")?;

    #[cfg(any(target_arch = "arm", target_arch = "aarch64"))] {
      let inference_io_mem = MemoryMappingBuilder::new(AARCH64_INFERENCE_IO_MEM_MAX_SIZE as usize)
          .build()?;
      if components.second_vm {
          vm.add_memory_region(
              GuestAddress(AARCH64_INFERENCE_IO_MEM_START_SECOND),
              Box::new(inference_io_mem),
              false,
              false,
              true,
          );
          ...
          vm.add_memory_region(
              GuestAddress(AARCH64_INFERENCE_IO_MEM_START),
              Box::new(inference_io_mem),
              false,
              false,
              true,
